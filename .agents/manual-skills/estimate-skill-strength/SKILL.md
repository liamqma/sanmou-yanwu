...
description: Estimate a 战法's per-round strength and write *Estimate fields onto its entry in web/public/game-data/database.json, organised into the categories healing/attribute/evasion/lifesteal/crit/critDamage. Each estimate is a rough per-round coefficient (max-level, average-trigger) so the team-builder prompt can compare skills. Use when the user asks to "estimate" / "估算" / "evaluate the strength of" a named 战法.
allowed-tools:
  - open_files
  - expand_code_chunks
  - grep
...
A skill can carry **several** of these at once (e.g. a buff that gives 属性 + 闪避). Only write
the categories the skill actually has. Direct damage and 增伤/减伤/降伤/易伤 are **not** estimated —
there are no database keys for them, so leave those components out.
...
The per-round estimate for a coefficient-style category (e.g. healing) is:
...
- **Averaging window = rounds 1–5.** For ramping / accumulating-DoT skills, average the per-round value
  over rounds 1–5. Use this so all skills are comparable.
...
- **Rounding:** integers for healing/attribute; one decimal is fine for small buff %s.
...
`attributeEstimate` (夺取 20武+20统 × 2 enemies); its direct damage is not estimated and the
conditional 40%会心+25%lifesteal rider is left un-estimated.
...
- **每战先登** (指挥, 先登 to self+1: +30先攻, +14%增伤, −14%减伤): `attributeEstimate 64.5`
  (`base × 2 × 8.6/8`); the 增伤/减伤 components are not estimated.
...
- **草船借箭** (指挥; 攻心+24%; 50% reactive 80% 谋): `lifestealEstimate 24`.
- **七进七出** (被动, +35%规避, 龙胆 reactive damage): `evasionEstimate 35`.
...
- **未雨绸缪** (60% 主动, 随机两人 140% 谋; 奇谋几率+奇谋伤害 +8%, +2%/功能性增益, 上限+8次):
  奇谋 buff recorded as sustained magnitude (8% base + ~2 增益 × 2%) → `critEstimate 12`,
  `critDamageEstimate 12`. The 获得1层抵御 single-hit block is left un-estimated (no clean per-round %).
