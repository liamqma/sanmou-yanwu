...
def export_stats(output_path: str = 'web/src/battle_stats.json'):
    ...
    stats = {
        'hero_stats': dict(ai.hero_stats),
        'skill_stats': dict(ai.skill_stats),
        'hero_combinations': {','.join(k): v for k, v in ai.hero_combinations.items()},
        'hero_pair_stats': {','.join(k): v for k, v in ai.hero_pair_stats.items()},
        'skill_pair_stats': {','.join(k): v for k, v in ai.skill_pair_stats.items()},
        'skill_hero_pair_stats': {f"{k[0]},{k[1]}": v for k, v in ai.skill_hero_pair_stats.items()},
