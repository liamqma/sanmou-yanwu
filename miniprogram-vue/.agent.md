...
```
Props: `items` (string[]), `modelValue` (string[]), `label`, `placeholder`, `title`, `max`, `tag-type` (`primary`|`success`|`warning`|`danger`), `search-placeholder`  
Search: case-insensitive, supports Chinese characters and pinyin prefix (e.g. `cao` → 曹操)  
Remove: selected items appear as tags below the trigger with a ✕ button — no `closable` prop needed, removal is always enabled
...
Test files: `setup.spec.js`, `itemPicker.spec.js`, `recommendation.spec.js` *(known flaky — pre-existing)*, `synergy.spec.js`, `promptGenerator.spec.js`
