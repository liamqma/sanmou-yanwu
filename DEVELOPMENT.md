...
| `agent/**` | **Agent checks**: `cd agent && pnpm typecheck && pnpm test && pnpm build`. Tests use fake providers and consume no model tokens. Run `pnpm smoke`, `pnpm recommend fixtures/partial-teams.json`, or `pnpm formation <hero-result.json>` only for an explicit live integration check when the local provider is available. |
