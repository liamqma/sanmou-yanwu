...
| `agent/**` | **Agent checks**: `cd agent && pnpm typecheck && pnpm test && pnpm build`. Tests use fake providers and consume no model tokens. Run `pnpm smoke` or `pnpm recommend -- fixtures/partial-teams.json` only for an explicit live integration check when the local provider is available. |
