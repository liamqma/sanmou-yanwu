...
| `study-battle-report/**` | **Isolated battle-log tests**: `make test-battle-logs` (no live model calls). Follow the [battle-log setup and validation commands](study-battle-report/README.md#requirements-and-commands) for this separate uv project. Validate model/alignment changes against all four local fixtures on Apple Silicon, inspect pending tags and boundary diagnostics, and verify deterministic cached replay in the same isolated output directory. |
