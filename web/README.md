...
`web/public/game-data/telemetry_data.json`. It does not upload the SQL export or
commit when the generated bytes are unchanged. Checkout does not persist its
GitHub credential; the workflow exposes that token only to the final push step.
