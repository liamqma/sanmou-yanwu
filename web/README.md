...
1. Add the `CLOUDFLARE_API_TOKEN` Actions secret. Use an account-scoped token
   ...
   secret is required. The current observation workflow does not use its write
...
The workflow exports the complete `round_telemetry` table to `$RUNNER_TEMP`,
runs the builder, then runs the web type-check, unit tests, and production build
against the current public schema-v3 artifact. The builder also creates or
advances `../data/telemetry_state.json` and validates it against the
independently rebuilt public artifact. Exactly that checkpoint and
...
export or commit when the generated bytes are unchanged. It reports only D1
size and the aggregate number of rows older than 14 days; no raw row is printed
or uploaded.
...
Raw-row deletion is completely disabled during this observation rollout. A
later, separately reviewed change may delete only rows that are both covered by
the committed checkpoint and older than 14 days. The aggregate checkpoint then
continues to represent validated historical telemetry; no R2/raw archive is
planned. Before enabling that change, migrate the current reusable D1 row ID to
`INTEGER PRIMARY KEY AUTOINCREMENT`, update the builder's canonical schema, and
verify delete-all followed by reinsertion so new rows can never fall behind the
committed cursor.
