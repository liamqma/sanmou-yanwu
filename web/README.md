...
The `Update telemetry data` GitHub Actions workflow runs every Monday at 03:17
UTC and can also be started manually. Configure it in the repository settings:
...
The workflow exports only `round_telemetry` to `$RUNNER_TEMP`, runs the
fail-closed builder, and stages only
`public/game-data/telemetry_data.json`. It does not upload the SQL export or
commit when the generated bytes are unchanged. Run the same path locally with:
