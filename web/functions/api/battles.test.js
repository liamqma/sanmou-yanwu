...
import recommendationData from '../../src/recommendation_data.json';
...
describe('battle submission validation', () => {
  ...
  test.each([
    ...
    ['钟会', '怀锋献策', 17],
    ['王平', '无当飞军', 17],
  ])('rejects %s before its introduction season', (name, signature, season) => {
    const lateHero = cloneBattle();
    lateHero['1'][0] = {
      name,
      skills: [signature, '避其锐气', '百战不殆'],
    };
    expect(
      validateBattleSubmission(
        submission({ battle: lateHero, season: season - 1 })
      )
    ).toBe(
      `battle.1[0].name is not available in season ${season - 1}`
    );
    expect(validateBattleSubmission(submission({ battle: lateHero, season })))
      .toBeNull();
  ...
  test.each(['保境安民', '随机应变'])('accepts %s only from S17', (skill) => {
    const battle = cloneBattle();
    battle['1'][0].skills[1] = skill;
    expect(validateBattleSubmission(submission({ battle, season: 16 })))
      .toBe('battle.1[0].skills contains a skill unavailable in season 16');
    expect(validateBattleSubmission(submission({ battle, season: 17 }))).toBeNull();
  });
...
describe('canonical battle fingerprints', () => {
  test('matches the catalog version generated by the Python data pipeline', async () => {
    await expect(getCatalogVersion()).resolves.toBe(recommendationData.catalog.catalog_version);
  });
  ...
  test('serializes only the exact compact battle contract in stable key order', () => {
    ...
    expect(canonicalBattleJson(BATTLE, CATALOG_MAX_SEASON)).toBe(
      JSON.stringify({
        '1': BATTLE['1'],
        '2': BATTLE['2'],
...
describe('POST /api/battles', () => {
  test('stores the exact uploader and treats a duplicate submission ID as idempotent', async () => {
    ...
    expect(stored[2]).toBe(recommendationData.catalog.catalog_version);
